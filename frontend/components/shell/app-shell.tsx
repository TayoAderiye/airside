...
function Topbar({ onMenu }: { onMenu: () => void }) {
  ...
  return (
    <header className="sticky top-0 z-30 flex h-12 items-center gap-3 border-b border-border bg-background/85 px-4 backdrop-blur sm:px-6 lg:px-8">
      <button
      ...
      <div className="flex min-w-0 items-baseline gap-2">
        ...
        <span className="truncate font-mono text-sm text-foreground">{hostName}</span>
      </div>
  ...
  return (
    <span
      className={cn(
        ...
        connected ? 'text-running' : 'text-failed',
      )}
      role="status"
    >
